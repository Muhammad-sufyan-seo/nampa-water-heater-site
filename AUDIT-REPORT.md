...
## Phase 9 — Direct-Provider Positioning, Logo Visibility, Why-Choose-Us Scoping, Real Brand Logos, Section De-Duplication (2026-08)

This phase addressed a 9-point follow-up review of Phase 8, confirmed with the site owner at two decision points (business-model change; brand logo sourcing) before implementing.

### Issue-by-issue changes

| # | Issue | Resolution |
|---|---|---|
| 1 | Referral/directory language | **Confirmed with the owner first** (via `AskUserQuestion`) that this is a real business-model change — Nampa Water Heater Pros now operates as a direct provider with its own licensed technicians, not a lead-gen/referral service — because the site's own `terms.html` explicitly stated the opposite ("we are not a licensed plumbing contractor... professionals we refer are independent contractors"), and rewriting marketing copy alone to contradict a live legal disclosure would have been a real compliance risk. With that confirmed, rewrote all "we connect you with licensed professionals" framing to direct first-person language across `index.html`, `about.html` (body copy, meta description, AboutPage schema, page-hero subtitle, FAQ schema + visible FAQ, feature-item copy), `contact.html`, both service pages that had it, **and** `terms.html` §"Nature of Our Service" + `privacy-policy.html`'s data-use/data-sharing sections, which previously described referring customer data to external "service providers." Recorded as `BIZ-MODEL-N009` in `rules/verified-claims.md` with an explicit note that the legal-page rewrite is not a substitute for attorney review. |
| 2 | Logo invisible on dark background | Added a white background container (`padding: 0.4rem 0.75rem`, `border-radius: 6px`) to the shared `.site-logo` class, so both header and footer logos now sit on a white card against the navy background. Verified at 375px (with a smaller 32px logo + tighter padding at ≤480px), 768px, and 1440px. |
| 3 | Why-Choose-Us on non-money pages | Removed the full section from `about.html`, `contact.html`, and all 6 common-issues pages (confirmed with the owner that common-issues pages — the closest thing this site has to blog posts — count as non-money for this purpose). Kept it only on the homepage, all 14 service pages, and all 4 service-area pages. |
| 4 | Why-Choose-Us customized per service | Rewrote the section's heading, intro, and all 6 feature items on every one of the 14 service pages to be genuinely specific to that service (e.g. the tankless repair page talks about error-code diagnosis and descaling; the heat pump installation page talks about ambient-air clearance and the federal tax credit) — while keeping the identical section structure, icon set, and heading style everywhere, so it still reads as one brand. |
| 5 | Sidebar widget for removed pages | Added a compact "Why Nampa Homeowners Trust Us" widget in place of the full section: a new `.sidebar-card` (reusing the site's existing checkmark-list pattern) in the 6 common-issues pages' sidebar and in `about.html`'s about-grid second column; a matching `.contact-detail` block inside `contact.html`'s dark `.contact-info-card`. |
| 6 | Homepage flow | Moved the "About Summary" section from between Services and Why-Choose-Us to immediately after the Hero section, before Services — confirmed via `check_h1.py` and a full-page screenshot. |
| 7 | Real brand logos | Initially found the site's `rheem-*.webp`-style files at only 205×72px and (incorrectly, on file-size assumption alone) assessed them as low-res product photos rather than logos. The owner corrected this — they *are* the real logo files. Re-inspected each one directly: all 7 (Rheem, A.O. Smith, Bradford White, American Standard, Navien, Rinnai, Noritz) are genuine brand wordmarks on white backgrounds. Replaced the text-card grid with real `<img>` logos (`object-fit: contain`, white card background, native 205×72 resolution — no upscaling). **"State" has no corresponding logo file and was dropped from the grid** (7 logos shown instead of 8); the body copy still lists "State" among brands serviced, which remains accurate independent of the logo grid. Flagging this gap in case the owner wants to supply a State logo file too. |
| 8 | Service-specific CTA button text | Homepage's 3 service-card buttons changed from generic "Repair Services" / "Installation Services" / "Replacement Services" to "Explore Repair Services" / "Book Installation Service" / "Get a Replacement Quote." Confirmed via `grep` that this `.service-card` pattern exists only on the homepage — no other page has a generic-CTA service-card grid needing the same fix. (Location-based CTAs like "Downtown Nampa Service Area →" were already specific and left unchanged.) |
| 9 | Duplicate Service Coverage / Map sections | Merged homepage's "Areas We Serve" (3 area cards + neighborhoods list) and "Our Service Area" (map + a second, redundant zip/description block) into one section: heading + intro + 3 cards + neighborhoods list + map, with a single one-line "Service Area Map — Primary ZIP Codes: ..." caption replacing the old duplicate paragraph. Audited the rest of the site for the same pattern and found two more instances, both fixed: **`contact.html`** had the service-area description repeated **three times** in the same page (a concise sidebar contact-detail, a longer "Service Area Block" restating it, and a third full heading + decorative photo + map) — collapsed to one concise sidebar mention plus a single map caption, and removed the unrelated decorative photo. **`about.html`** had a redundant "Service Area Business" paragraph restating the section's own intro — shortened to just the one distinct fact (no public storefront) plus the zip codes. Checked `service-areas/index.html` and the three individual area pages (`downtown-nampa.html`, `central-nampa.html`, `south-nampa.html`) for the same pattern — none found; their sections cover genuinely distinct topics. **Bonus defect found and fixed during this audit** (pre-existing, not introduced by this phase): `about.html`'s entire FAQ section (4 Q&A pairs, matching its `FAQPage` schema) was nested *inside* the "Downtown Nampa" area-card `<article>` instead of being its own top-level section — almost certainly a copy-paste/merge artifact from an earlier edit. Moved it out to a proper standalone `<section>` in the page's normal content flow, matching the pattern used on `index.html` and every service page. |
...
| `scripts/validate_schema.py` | ✅ 86/86 JSON-LD blocks valid |
| `scripts/check_phone.py` | ✅ all tel:/schema phone formats correct |
| `scripts/check_h1.py` | ✅ 29/29 pages, one unique H1 each |
| `scripts/check_faq_parity.py` | ✅ 25/25 FAQ-schema pages match visible text word-for-word |
| Referral-language sweep (session one-off) | ✅ 0 remaining instances outside `terms.html`'s explicit negation statement |
| Image width/height audit (session one-off) | ✅ 0 of 111 `<img>` tags missing dimensions |
| Live-browser visual check (Playwright/Chromium) | ✅ logo white-bg at 375/768/1440px, customized Why-Choose-Us (tankless page), reordered homepage flow, real brand-logo grid (desktop + mobile), homepage CTA text, merged area/map section, about.html FAQ fix, contact.html map fix — all verified rendering correctly |

### Not done / scope notes

- **"State" brand logo** — flagged above (Issue 7); no source file exists, needs the owner to supply it if they want all 8 brands shown as logos.
- **`terms.html` / `privacy-policy.html` rewrite** — reflects the confirmed business-model change accurately, but is not a substitute for review by a licensed Idaho attorney before the business relies on this language in production (see `BIZ-MODEL-N009` in `rules/verified-claims.md`).
